...
export const handler: Handler = async (event: IEmail) => {
  const attributes = Object.entries(event.attributes)
	if (!attributes || attributes.length <= 0){
		throw new Error(JSON.stringify(['No message in queue or no message attributes', event]))
	}
	...
	if(!email || !uid || !expiration){
		throw new Error(JSON.stringify(['Error response from event bridge, stopping queue', event, attributes, Aws.STACK_NAME]))
	}
	...
	// deleteMessageParamsdeleteMessageParams
	console.log([event, sesResponse])
  return JSON.stringify([event, sesResponse])
}