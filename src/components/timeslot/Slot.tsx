...
export function createTimeslotEvent(timeslot: Timeslot, participant: Participant) {
  ...
  return (
    <a href={objectURL} download={`${participant.firstName}_${participant.lastName}.ics`}>
      <HiOutlineCalendar size={24} />
    </a>
  )
}

export const SlotComponent: FC<SlotProps> = ({ timeslot, participant, className }) => {
  return (
    <div className={`font-sans flex flex-col relative border border-black justify-center items-center rounded-lg py-2 px-4 text-${timeslot.tag?.color ?? 'black'} ${className ?? ''}`}>
      <div className="flex flex-row gap-1 font-light">
        <span>{formatTime(timeslot.start, { timeString: false })}</span>
        {timeslot.description !== undefined && timeslot.description !== '' && (
          <>
            <span>&bull;</span>
      ...
      <span className="font-medium whitespace-nowrap">{formatTimeslotDates(timeslot)}</span>
      {participant !== null && (
        participant ? (
