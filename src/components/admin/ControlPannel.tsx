...
export const ControlComponent: FC<ControlComponentProps> = ({name, fn, disabled, type, className}) => {
    if(type){
        return (
            <button className={`flex flex-row items-center hover:bg-gray-100 ps-2 py-1 rounded-3xl cursor-pointer disabled:text-gray-400 ` + className} disabled={disabled} onClick={() => {fn()}}>
                {name}
            </button>
        )
    }
