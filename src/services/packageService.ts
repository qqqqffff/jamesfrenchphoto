...
export class PackageService {
  ...
  async createPackageMutation(params: CreatePackageParams) {
    const start = new Date()
    const createPackageResponse = this.client.models.Package.create({
      id: params.pack.id,
    ...
    if(params.pack.parentTagId) {
      const createParentTagging = this.client.models.PackageParentTag.create({
    ...
    const createPackageItemsResponses = Promise.all(params.pack.items.map(async (item) => {
      const collectionItemsResponses = Promise.all(item.collectionIds.map((id) => {
        return this.client.models.PackageItemCollection.create({ 
          collectionId: id, 
      ...
      return [
        ...
        this.client.models.PackageItem.create({
          id: item.id,
          name: item.name,
          packageId: params.pack.id,
          order: item.order,
  ...
  async updatePackageMutation(params: UpdatePackageParams) {
    const start = new Date()
    ...
    Object.fromEntries(await Promise.all(params.pack.items.map(async (item) => {
      let collectionItemsResponse = await this.client.models.PackageItemCollection
      ...
      while(collectionItemsResponse.nextToken) {
        collectionItemsResponse = await this.client.models.PackageItemCollection
    ...
    const itemUpdatesResponse = Promise.all(params.items.map(async (item) => {
      if(newItems[item.id]) {
        const collectionItemsResponses = Promise.all(item.collectionIds.map(async (id) => {
          return this.client.models.PackageItemCollection.create({ 
            collectionId: id, 
            packageItemId: item.id 
          })
        ...
        const removedCollectionItems = Promise.all(packItems[item.id].collectionIds
          ...
          .map((id) => {
            ...
            if(foundConnectionId) {
              return this.client.models.PackageItemCollection.delete({ id: foundConnectionId })
            }
          })
        )

        const newCollectionItems = Promise.all(item.collectionIds
        ...
        return [
          ...
          newCollectionItems,
          this.client.models.PackageItem.update({
            id: item.id,
            name: item.name,
            packageId: params.pack.id,
            order: item.order,
    ...
    const removedItemsResponse = await Promise.all(removedItems.map(async (item) => [
      this.client.models.PackageItem.delete({ id: item.id }),
      Promise.all(currentItemConnections[item.id].map((connection) => this.client.models.PackageItemCollection.delete({ id: connection[0] })))
    ]))
    ...
    if(
      ...
      const response = this.client.models.Package.update({
        id: params.pack.id,
        name: params.name,
  ...
  async deletePackageMutation(params: DeletePackageParams) {
    const start = new Date()

    const deletePackResponse = this.client.models.Package.delete({ id: params.pack.id })
    if(params.options?.logging) console.log(deletePackResponse)

    const itemConnectionDeletions = Promise.all(params.pack.items.map(async (item) => {
      let connectionResponse = await this.client.models.PackageItemCollection
        .listPackageItemCollectionByPackageItemId({ packageItemId: item.id })
      const connectionData = connectionResponse.data

      while(connectionResponse.nextToken) {
        connectionResponse = await this.client.models.PackageItemCollection
        .listPackageItemCollectionByPackageItemId({ 
          packageItemId: item.id 
        }, { 
          nextToken: connectionResponse.nextToken 
        })
      ...
      return [
        this.client.models.PackageItem.delete({ id: item.id }),
        Promise.all(connectionData.map((connection) => (
          this.client.models.PackageItemCollection.delete({ id: connection.id })
        )))
      ]
    }))
  ...
  getPackagesByUserTagsQueryOptions = (tags: UserTag[]) => queryOptions({
    queryKey: ['package', tags],
    queryFn: () => getPackagesByUserTags(this.client, tags)
  })

  getInfinitePackagesQueryOptions = (options?: GetInfinitePackagesOptions) => infiniteQueryOptions({
    queryKey: ['infinitePackages', options],
    queryFn: ({ pageParam }) => getInfinitePackages(this.client, pageParam, options),
  ...
  getAllPackagesQueryOptions = (options?: GetAllPackagesOptions) => queryOptions({
    queryKey: ['package', options],
    queryFn: () => getAllPackages(this.client, options)
  })

  getInfinitePackageItemsQueryOptions = (options?: GetInfinitePackageItemsOptions) => infiniteQueryOptions({
    queryKey: ['infinitePackageItems', options],
    queryFn: ({ pageParam }) => getInfinitePackageItems(this.client, pageParam, options),
    getNextPageParam: (lastPage) => lastPage.nextToken ? lastPage : undefined,
    initialPageParam: ({
      memo: [] as PackageItem[]
  ...
  getAllPackageItemsQueryOptions = (packageId?: string, options?: GetAllPackageItemsOptions) => queryOptions({
    queryKey: ['packageItems', options],
    queryFn: () => getAllPackageItems(this.client, packageId, options)
  })

  getPackageDataFromPathQueryOptions = (path: string) => queryOptions({
